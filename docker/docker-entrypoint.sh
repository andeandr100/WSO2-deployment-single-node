...
then
  ...
  # debug ssl -Djavax.net.debug=all:handshake:verbose
  # nohup ./wso2server.sh -Djavax.net.debug=ssl:handshake &

  #nohup ${WSO2_SERVER_HOME}/bin/api-manager.sh -Djavax.net.debug=ssl:handshake "$@" &> ${WSO2_SERVER_HOME}/logs.out
  sh ${WSO2_SERVER_HOME}/bin/api-manager.sh "$@"
else
  # start the server with the specified profile and provided startup arguments

  #nohup ${WSO2_SERVER_HOME}/bin/api-manager.sh -Dprofile=${PROFILE_NAME} -Djavax.net.debug=ssl:handshake "$@" &> ${WSO2_SERVER_HOME}/logs.out
